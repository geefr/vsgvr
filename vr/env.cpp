...
void Env::update() {
  ...
  while (mContext->PollNextEvent(&ev, sizeof(ev))) {
    if (ev.eventType == vr::VREvent_TrackedDeviceActivated ||
      ...
      if (mLogging)
        std::cout << "EVENT: Device activated/Updated: TYPE: " << ev.eventType
                  << " DEV: " << ev.trackedDeviceIndex << std::endl;
    } else if (ev.eventType == vr::VREvent_TrackedDeviceDeactivated) {
      mDevices.erase(ev.trackedDeviceIndex);
      if (mLogging)
        std::cout << "EVENT: Device Deactivated DEV: " << ev.trackedDeviceIndex
                  << std::endl;
    } else {
      ...
      case vr::VREvent_ButtonPress:
        ...
        if (mLogging)
          std::cout << "EVENT: ButtonPress: DEV: " << ev.trackedDeviceIndex
                    << " BUTTON: " << ev.data.controller.button << std::endl;
        break;
      case vr::VREvent_ButtonUnpress:
        controller->buttonUnPress(ev.data.controller.button);
        if (mLogging)
          std::cout << "EVENT: ButtonUnPress: DEV: " << ev.trackedDeviceIndex
                    << " BUTTON: " << ev.data.controller.button << std::endl;
        break;
      case vr::VREvent_ButtonTouch:
        controller->buttonTouch(ev.data.controller.button);
        if (mLogging)
          std::cout << "EVENT: ButtonTouch: DEV: " << ev.trackedDeviceIndex
                    << " BUTTON: " << ev.data.controller.button << std::endl;
        break;
      case vr::VREvent_ButtonUntouch:
        controller->buttonUntouch(ev.data.controller.button);
        if (mLogging)
          std::cout << "EVENT: ButtonUntouch: DEV: " << ev.trackedDeviceIndex
                    << " BUTTON: " << ev.data.controller.button << std::endl;
        break;
      default:
        break;
...
vrhelp::Controller *Env::leftHand() const {
  ...
  auto dev = std::find_if(mDevices.begin(), mDevices.end(), [&](const auto &d) {
    return dynamic_cast<vrhelp::Controller *>(d.second.get());
  });
  if (dev == mDevices.end())
    return nullptr;
...
vrhelp::Controller *Env::rightHand() const {
  ...
  // Should make better use of openvr here
  auto dev =
      std::find_if(mDevices.rbegin(), mDevices.rend(), [&](const auto &d) {
        return dynamic_cast<vrhelp::Controller *>(d.second.get());
      });
...
vrhelp::Device *Env::device(uint32_t id) {
  auto it = std::find_if(mDevices.begin(), mDevices.end(),
                         [&](const auto &d) { return d.second->mID == id; });
  if (it == mDevices.end())
    return nullptr;
...
std::vector<glm::dmat4x4> Env::getProjectionMatrices(float nearZ, float farZ) {
  return {getProjectionMatrix(vr::EVREye::Eye_Left, nearZ, farZ),
          getProjectionMatrix(vr::EVREye::Eye_Right, nearZ, farZ)};
}

std::vector<glm::dmat4x4> Env::getEyeToHeadTransforms() {
  return {getEyeToHeadTransform(vr::EVREye::Eye_Left),
...
glm::dmat4x4 Env::getEyeToHeadTransform(vr::EVREye eye) {
  ...
  auto m = transform.m;
  return {
      m[0][0], m[1][0], m[2][0], 0.0f, m[0][1], m[1][1], m[2][1], 0.0f,
      m[0][2], m[1][2], m[2][2], 0.0f, m[0][3], m[1][3], m[2][3], 1.0f,
  };
}
...
void Env::submitFrames(GLuint leftTex, GLuint rightTex) {
  ...
  vr::Texture_t leftVRTex{reinterpret_cast<void *>(leftTex),
                          vr::ETextureType::TextureType_OpenGL, cSpace};
  vr::Texture_t rightVRTex{reinterpret_cast<void *>(rightTex),
                           vr::ETextureType::TextureType_OpenGL, cSpace};
...
std::list<std::string> Env::instanceExtensionsRequired() const {
  ...
  uint32_t nBufferSize =
      vr::VRCompositor()->GetVulkanInstanceExtensionsRequired(nullptr, 0);
  if (nBufferSize > 0) {
    // Allocate memory for the space separated list and query for it
    char *pExtensionStr = new char[nBufferSize];
    pExtensionStr[0] = 0;
    vr::VRCompositor()->GetVulkanInstanceExtensionsRequired(pExtensionStr,
                                                            nBufferSize);
...
std::list<std::string>
Env::deviceExtensionsRequired(VkPhysicalDevice physicalDevice) const {
  if (!vr::VRCompositor())
    return {};

  std::list<std::string> extensions;
  uint32_t nBufferSize = vr::VRCompositor()->GetVulkanDeviceExtensionsRequired(
      physicalDevice, nullptr, 0);
  if (nBufferSize > 0) {
    // Allocate memory for the space separated list and query for it
    char *pExtensionStr = new char[nBufferSize];
    pExtensionStr[0] = 0;
...
void Env::submitFrames(const std::vector<VkImage> images, VkDevice device,
...
void Env::waitGetPoses() {
  ...
  // If we don't wait here the compositor doesn't accept the frames we're trying
  // to give it Of course that does mean we're probably setting all the
  // transforms/etc at the wrong point...
  vr::VRCompositor()->WaitGetPoses(mTrackedDevicePoses,
